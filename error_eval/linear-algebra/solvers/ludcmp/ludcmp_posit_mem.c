...
void init_array (int n,
		 ...
		 MEM_TYPE POLYBENCH_1D(b_mem,N,n),
...
{
  int i, j;
  MEM_TYPE fn = (MEM_TYPE)n;

  for (i = 0; i < n; i++)
    {
      x[i] = 0;
      y[i] = 0;
      b_mem[i] = (i+1)/fn/2.0 + 4;
      b[i] = (DATA_TYPE)(b_mem[i]);
    }
  ...
  for (t = 0; t < n; ++t)
    ...
    for (r = 0; r < n; ++r)
      for (s = 0; s < n; ++s)
      {
        A_mem[r][s] = (POLYBENCH_ARRAY(B))[r][s];
        A[r][s] = (DATA_TYPE)(A_mem[r][s]);
      }
...
int main(int argc, char** argv)
{
  ...
  POLYBENCH_1D_ARRAY_DECL(b_mem, MEM_TYPE, N, n);
  POLYBENCH_1D_ARRAY_DECL(x_mem, MEM_TYPE, N, n);
  ...
  /* Prevent dead-code elimination. All live-out data must be printed
	...
	for (int i = 0; i < n; i++)
			(POLYBENCH_ARRAY(x_mem))[i] = (MEM_TYPE)((POLYBENCH_ARRAY(x))[i]);
  polybench_prevent_dce(print_array(n, POLYBENCH_ARRAY(x_mem)));

  /* Be clean. */
  POLYBENCH_FREE_ARRAY(A_mem);
  POLYBENCH_FREE_ARRAY(b_mem);
  POLYBENCH_FREE_ARRAY(x_mem);
